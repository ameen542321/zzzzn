...
<div class="max-w-4xl mx-auto p-6 space-y-6" dir="rtl">
    <div class="flex items-center justify-between gap-3">
        <div>
            <h1 class="text-2xl font-black text-white">إنشاء طلب نقل مخزني</h1>
            <p class="text-gray-400 text-sm mt-1">سيتم خصم الكمية من متجر {{ $store->name }} فورًا وحجزها حتى يوافق المستلم.</p>
        </div>
        <a href="{{ route('user.stores.transfers.index', $store->id) }}" class="px-4 py-2 rounded-lg bg-gray-800 text-gray-200 hover:bg-gray-700">رجوع</a>
    </div>
    ...
    <form method="POST" action="{{ route('user.stores.transfers.store', $store->id) }}" class="bg-gray-900 border border-gray-800 rounded-2xl p-6 space-y-5">
        @csrf
        ...
        <div class="rounded-xl border border-amber-500/20 bg-amber-500/10 p-4 text-sm text-amber-100">
            لا يتم اختيار منتج المتجر المستلم الآن. عند الاستلام سيختار المحاسب أو المالك المنتج المقابل، وإذا لم يكن موجودًا يجب إنشاؤه أولًا. منتجات الرول تظهر بوحدتي رول كامل أو متر.
        </div>

        <button class="w-full rounded-xl bg-blue-600 hover:bg-blue-500 text-white font-black py-3">إنشاء طلب النقل</button>
    </form>
</div>
