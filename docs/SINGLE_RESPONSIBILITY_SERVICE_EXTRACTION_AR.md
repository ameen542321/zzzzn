...
## نطاق العمل المتبقي الآن

بعد حذف البنود التي اكتملت من قائمة التنفيذ، يبقى هذا المستند مخصصًا للأعمال التي تحتاج نقلًا أو توحيدًا إضافيًا فقط:

- استكمال خدمة عمليات مالية مشتركة لـ `Sale`, `Expense`, و`Withdrawal`، ومتابعة توحيد تاريخ عمليات الموظف عبر `forOperationDate` و`betweenOperationDates` بدل `created_at` في الغياب والمديونيات والآجل والتحصيلات.
- توحيد Presenter صف العملية المالية في المواضع التي لا تزال تنسق العملية داخل Controller أو Blade.
- مراجعة خدمات الصفحات التي بقيت طويلة أو خاصة بواجهة واحدة، بعد نقل تسجيل عمليات الموظف الأساسية وتحصيل المديونيات والآجل إلى `EmployeeOperationService`.
- الواجهات تدخل في هذا المستند عندما يكون العمل تفكيك Blade إلى Components/Partials أو نقل تجهيز العرض إلى Presenter/ViewModel؛ أما اختبار سلوك عمليات الموظف نفسها فيبقى في مستند مراجعة عمليات الموظفين والمحاسبين.
- الاستمرار في حذف الدوال القديمة بعد فحص الاستدعاءات وإضافة مقارنة قبل/بعد للمسارات المالية الحساسة.
...
- التطبيق السابق للمستند حقق نقل كتل مهمة من الكنترولرات، لكنه بدأ يميل إلى إنشاء خدمات مرتبطة بصفحات أو كنترولرات محددة مثل `StoreDashboardService` بدل بناء خدمات مفاهيمية مشتركة دائمًا.
...
- مراجعة `StoreDashboardService` و`StoreDetailsService`.
...
| إحصاءات المخزون والموظفين داخل خدمات المتجر | مرشحة للمراجعة | خدمة مفاهيمية مشتركة عند تكرار الاستخدام | لا ينشأ ملف خدمة جديد إلا إذا خدم أكثر من شاشة أو قلل تكرارًا حقيقيًا |
