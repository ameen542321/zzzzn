...
## 0. نطاق العمل المتبقي الآن

بعد حذف البنود التي اكتملت من هذه الخارطة، يبقى التركيز التنفيذي على الآتي فقط:

- استكمال نقل بقية القيم المالية إلى DTOs بعد بدء اعتماد `FinancialSummaryResult` و`StoreFinancialSummary` في خدمة الملخص المالي.
- إضافة اختبارات تثبيت ومقارنات قبل/بعد للمسارات المالية الحساسة.
- استكمال ربط أي مسارات مالية قديمة خارج لوحة المالية/لوحة المتجر/التقرير الشهري بالمصدر المالي المشترك.
- نقل المديونيات والبيع الآجل والتحصيلات إلى خدمات مشتركة وربط أثرها بخدمة الملخص المالي دون إدخالها في كاش الشفت افتراضيًا.
- حذف الحسابات القديمة بعد المقارنة وليس بمجرد وجود خدمة بديلة.

**تقدم الخطوة التالية:** بدأت اختبارات التثبيت من طبقة DTO بإضافة اختبار `FinancialSummaryResult` و`StoreFinancialSummary` لضمان ثبات جمع الإجماليات ومعادلة الربح، ثم تم نقل لوحة المالك وتفاصيل المتجر والتقرير الشهري إلى `storeSummariesForPeriod` وحذف جسر المصفوفات القديم. وتم ربط مؤشرات ذمم الموظفين غير النقدية (`employee_debt_balance`, `employee_credit_outstanding`, `employee_credit_collections`) بخدمة الملخص المالي مع تثبيت أنها لا تدخل في ربح المتجر أو كاش الشفت افتراضيًا.

### واجهات مالية تحتاج تجربة ومقارنة أرقام

هذه الواجهات ضمن نطاق هذه الخارطة فقط من زاوية **مطابقة الأرقام المالية**، وليست من زاوية تصميم الواجهة:

| الواجهة | ما يجب مقارنته |
|---|---|
| لوحة المالك | المبيعات المحصلة، تكلفة المنتجات، الربح، المصروفات، والمشتريات لنفس الفترة والمتجر |
| لوحة المتجر | مبيعات اليوم والشهر، الفواتير، أفضل المنتجات، وربح آخر 7 أيام |
| التقرير الشهري للمتجر | صافي المبيعات، تكلفة المنتجات، المصروفات، الربح، والموظفون التاريخيون بعد النقل |
| التقارير/PDF/WhatsApp | أن تطابق أرقامها مصدر `FinancialSummaryService` أو توثق سبب اختلافها |
| إغلاق الشفت | أن تتطابق العمليات المرتبطة بالشفت مع تاريخ `business_date` وليس وقت الإدخال فقط |
...
يجب اعتماد وثيقة القواعد قبل توسيع `FinancialSummaryService` أو حذف أي حساب قديم يعتمد عليه تقرير فعلي.
...
## المرحلة 3: استكمال DTOs للنتائج
...
تم بدء ذلك في ملخصات المتجر عبر DTOs، والمتبقي نقل بقية المسارات مثل `SaleAmounts` و`SaleProfit` تدريجيًا بدل المصفوفات غير الموحدة.
...
- تم استخراج التقرير الشهري وتقرير البحث الشامل إلى خدمات تقارير وربطهما بالمصدر المالي المشترك؛ المتبقي هو إضافة اختبارات مقارنة وحذف الحسابات القديمة بعد ثبوت التطابق.
...
- تم ربط لوحة المالك ولوحة التقارير المالية العامة وصفحة تفاصيل المتجر بالمصدر المالي المشترك؛ المتبقي هو الواجهات والتقارير التي لم تُنقل بعد.
...
- توجد خدمات صفحة مثل `StoreDashboardService`، بينما الهدف المالي هو `FinancialSummaryService`; يجب ألا تصبح خدمات الصفحة مصدرًا ماليًا موازيًا.
...
3. استكمال تطوير خدمة مشتركة لجلب العمليات المالية لفترة أو شفت.
4. إضافة Presenter موحد لصف العملية المالية حيث لا يزال التنسيق داخل الكنترولرات أو Blade.
5. إضافة مقارنة قبل/بعد على آخر العمليات وتفاصيل الشفت والمبيعات اليومية.
6. بعد نجاح المقارنة، تُحذف الحسابات القديمة المتبقية بدل إبقائها كمسارات موازية.
...
- المتبقي ماليًا هو ربط إغلاق الشفت وأي تقارير غير منقولة بنفس تعريف النافذة ثم مقارنة النتائج قبل حذف المنطق القديم.
...
- خدمات صفحة المتجر تبقى للعرض، أما مصدر الحقيقة المالي فهو الخدمة المالية المشتركة؛ أي منطق مالي مكرر داخل خدمات الصفحة يجب نقله أو حذفه بعد المقارنة.
...
- هذا يمنع أخطاء النطاق والتفسير قبل حذف الحسابات القديمة المتبقية.
