...
- توسيع `SalesCostService` ليحسب تكلفة المنتجات لمتجر واحد أو عدة متاجر، واستخدامه في لوحة المالك بدل بقاء معادلة التكلفة داخل الكنترولر.
- وجود خدمات تقارير متجر منفصلة، خصوصًا التقرير الشهري والبحث الشامل وملفات التقارير الحديثة.

**ما لم يكتمل بعد:** `FinancialSummaryService` و DTOs المالية المركزية، اختبارات التثبيت المقترحة، توسيع اعتماد الخدمات الجديدة في كل المسارات، توحيد الكنترولرات والواجهات على الخدمات، وحذف الحسابات القديمة بعد المقارنة.
