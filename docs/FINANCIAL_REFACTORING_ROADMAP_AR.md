...
- توسيع `SalesCostService` ليحسب تكلفة المنتجات لمتجر واحد أو عدة متاجر، واستخدامه داخل `FinancialSummaryService` بدل بقاء معادلة التكلفة داخل الكنترولر.
- إنشاء `FinancialSummaryService` كبداية للمصدر المالي المشترك لملخصات لوحة المالك اليومية والشهرية وبطاقات المتاجر، وربطه بالتقرير الشهري للمتجر وتقرير البحث الشامل وصفحة تفاصيل المتجر وتقارير الاستهلاك الداخلي ولوحة التقارير المالية العامة.
- وجود خدمات تقارير متجر منفصلة، خصوصًا التقرير الشهري والبحث الشامل وملفات التقارير الحديثة، مع بدء حذف الدوال والخدمات الخاملة بعد نقل المسؤوليات للخدمات المشتركة، وتوحيد فلترة مصروفات المتجر الشهرية على `betweenAccountingDates`، ونقل لوحة التقارير المالية العامة إلى `FinancialSummaryService`.

**ما لم يكتمل بعد:** DTOs المالية المركزية، اختبارات التثبيت المقترحة، توسيع اعتماد `FinancialSummaryService` في بقية التقارير والواجهات غير المنقولة بعد، توحيد الكنترولرات والواجهات على الخدمات، وحذف الحسابات القديمة بعد المقارنة.
