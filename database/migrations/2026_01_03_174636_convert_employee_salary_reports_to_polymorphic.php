   ...
   public function up()
{
    Schema::table('employee_salary_reports', function (Blueprint $table) {
        ...
        if (Schema::hasColumn('employee_salary_reports', 'employee_id')) {
            $table->renameColumn('employee_id', 'person_id');
        }

        // 2) إضافة person_type
        if (!Schema::hasColumn('employee_salary_reports', 'person_type')) {
            $table->string('person_type')->nullable()->after('person_id');
        }
    });

    // 3) تحديث البيانات القديمة
    DB::table('employee_salary_reports')
...
{
    Schema::table('employee_salary_reports', function (Blueprint $table) {
        if (Schema::hasColumn('employee_salary_reports', 'person_id')) {
